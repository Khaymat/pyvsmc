"""Adversarial FVG benchmark — pathological Θ(n) gaps with d≈n.
...
def make_adversarial(n: int):
    ...
    low = np.full(n, 11.0, dtype=np.float64)
    close = np.full(n, 10.5, dtype=np.float64)
    # Ensure first two bars not gaps but not important
    # Last bar mitigates all: low final = 9 <= 11
    if n > 0:
        low[-1] = 9.0
        close[-1] = 9.0
...
def main():
    ...
    print(f"pyvsmc {pyvsmc.__version__} adversarial FVG — g=Θ(n), d≈n/2, all mitigate at last bar")
    ...
    print("Typical random 100k fvg_mit ≈16ms (57× vs 0.3.1) for d small; adversarial worst remains Θ(n²).")
